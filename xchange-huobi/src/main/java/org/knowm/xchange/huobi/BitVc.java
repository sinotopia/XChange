...
public interface BitVc {
    ...
    @POST
    @Path("api/order/{id}")
    HuobiOrder getOrder(@FormParam("access_key") String accessKey, @FormParam("coin_type") int coinType,
                        @FormParam("created") long created, @FormParam("sign") ParamsDigest sign, @PathParam("id") long id) throws IOException;
